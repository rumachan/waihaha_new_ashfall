...
set web_directory = /var/www/html/volcano/new_ashfall
...
#remove all files from web_dir so new files can go in later
\rm $web_directory/*
#copy new wind file to web_dir
\cp $metwindfile $web_directory/current_wind_file.txt
...
foreach volcano ( $volcano_name )
	...
	foreach windfile ($wind_files)
		...
		foreach volfile (`ls $vol_directory/*$vol_ident*`)
			...
			#gns logo
			composite -geometry +750+250 $program_directory/GNS_logo_small.png $plotfile:r.gif $output_directory/a.gif
			\mv $output_directory/a.gif $plotfile:r.gif 
			#end gns logo
		end
		#create kml files
